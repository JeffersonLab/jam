...
public class BeamDestinationFacade extends AbstractFacade<BeamDestination> {
  ...
  public List<BeamDestination> findActiveDestinations() {
    Query q =
        em.createNativeQuery(
            "select * from JAM_4202C_OWNER.beam_destination where ACTIVE_YN = 'Y' order by weight",
  ...
  public List<BeamDestination> findCebafDestinations() {
    Query q =
        em.createNativeQuery(
            "select * from JAM_4202C_OWNER.beam_destination where machine = 'CEBAF' and ACTIVE_YN = 'Y' order by weight",
            BeamDestination.class);
  ...
  public List<BeamDestination> findLerfDestinations() {
    Query q =
        em.createNativeQuery(
            "select * from JAM_4202C_OWNER.beam_destination where machine = 'LERF'  and ACTIVE_YN = 'Y' order by weight",
            BeamDestination.class);
  ...
  public List<BeamDestination> findUitfDestinations() {
    Query q =
        em.createNativeQuery(
            "select * from JAM_4202C_OWNER.beam_destination where machine = 'UITF'  and ACTIVE_YN = 'Y' order by weight",
            BeamDestination.class);
