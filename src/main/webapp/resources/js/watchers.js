...
jlab.editableRowTable.entity = 'Watcher';
...
jlab.addRow = function() {
    ...
    var request = jQuery.ajax({
        url: jlab.contextPath + "/ajax/add-watcher",
        type: "POST",
        data: {
            username: username,
            type: type,
    ...
    request.fail(function(xhr, textStatus) {
        window.console && console.log('Unable to add watcher; Text Status: ' + textStatus + ', Ready State: ' + xhr.readyState + ', HTTP Status Code: ' + xhr.status);
...
jlab.removeRow = function() {
    ...
    var request = jQuery.ajax({
        url: jlab.contextPath + "/ajax/remove-watcher",
        type: "POST",
        data: {
            facilityId: facilityId,
            username: username,
    ...
    request.fail(function(xhr, textStatus) {
        window.console && console.log('Unable to remove watcher; Text Status: ' + textStatus + ', Ready State: ' + xhr.readyState + ', HTTP Status Code: ' + xhr.status);
...
$(document).on("click", "#remove-row-button", function() {
    ...
    if (confirm('Are you sure you want to remove ' + name + ' from ' + facility + ' watcher of type ' + type + '?')) {
        jlab.removeRow();
    }
});
